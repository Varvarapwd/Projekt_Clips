...
def update_textes(start=False):
    ...
    if not start:
        if state == 'final':
            question.configure(font=('Comic Sans MS', 18, 'italic'), padx=10, pady=10, fg='#e9736a', bd=3)
        else:
            question.configure(textvariable=text_question, padx=1, pady=7, bg='#ADD8E6', fg='#0000ff', font=('Comic Sans MS', 12, 'italic'), bd=1, relief=FLAT)
...
if __name__ == '__main__':
    ...
    for _ in range(2):
        ...
        button = Button(root, textvariable=text_var, width=90, padx=2, pady=2, command=lambda i=_: ans_button_command(i), borderwidth=3, activebackground='#9ada7d', bg='#FFB6C1', fg='#0b0b0b', font=('Comic Sans MS', 10, 'italic'))
        all_buttons.append(button)
        button.grid_remove()

    update_textes(start=True)

    root.title('What should You do this summer?')

    question = Label(root, textvariable=text_question, pady=7, bg='#ADD8E6', fg='#0000ff', font=('Comic Sans MS', 12, 'italic'))
    empty_space = Label(root, text='', bg='#ADD8E6', height=2)

    button_back = Button(root, textvariable=text_button_back, width=90, padx=2, pady=2, command=back_button_command, borderwidth=4, activebackground='#9ada7d', bg='#FFB6C1', fg='#0b0b0b', font=('Comic Sans MS', 10, 'italic'))
